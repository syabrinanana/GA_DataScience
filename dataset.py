...
def load_train(train_path, image_size, classes):
    ...
    for fld in classes:   # assuming data directory has a separate folder for each class, and that each folder is named after the class
        ...
        for fl in files:
            image = cv2.imread(fl)
            # print(type(image))
            image = cv2.resize(src=image, dsize=(image_size, image_size), interpolation=cv2.INTER_LINEAR)
            images.append(image)
...
#       img = cv2.imread(fl)
#     #   print(img)
#       img = cv2.resize(src=img, dsize=(image_size, image_size), interpolation=cv2.INTER_LINEAR)
#       X_test.append(img)
